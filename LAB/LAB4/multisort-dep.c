...
void merge(long n, T left[n], T right[n], T result[n*2], long start, long length) {
    if (length < MIN_MERGE_SIZE*2L) {
        ...
        #pragma omp task depend(in: left[0], right[0]) depend(out: result[0])
        merge(n, left, right, result, start, length/2);
        #pragma omp task depend(in: left[length/2], right[length/2]) depend(out: result[length/2])
...
void multisort(long n, T data[n], T tmp[n]) {
    if (n >= MIN_SORT_SIZE*4L) {
        // Recursive decomposition
        #pragma omp task depend(out: tmp[0],data[0])
        multisort(n/4L, &data[0], &tmp[0]);
        #pragma omp task depend(out: tmp[n/4L], data[n/4L])
        multisort(n/4L, &data[n/4L], &tmp[n/4L]);
        #pragma omp task depend(out: tmp[n/2L], data[n/2L])
        multisort(n/4L, &data[n/2L], &tmp[n/2L]);
        #pragma omp task depend(out: tmp[3L*n/4L], data[3L*n/4L])
